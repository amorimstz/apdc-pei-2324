...
public class ChangeStateResource {
	...
	public Response changeUserState(@HeaderParam(HttpHeaders.AUTHORIZATION) String authTokenHeader,
		...
		try {
			...
			// Check if the user has the necessary permissions to change states
			if (!hasPermissionToChangeState(user, userToChange)) {
				LOG.warning("User does not have permission to change states.");
				return Response.status(Status.FORBIDDEN).entity("You don't have permission to change states.").build();
			}

			user = Entity.newBuilder(userKey).set("user_state", data.newState).build();
			txn.update(user);
			LOG.info("User state changed to " + data.newState);
			txn.commit();
	...
	private boolean hasPermissionToChangeState(Entity user, Entity userToChange) {
		...
		if (userRole.equals("SU")) {
			...
			return true;
		} else if (userRole.equals("GBO") && userToChangeRole.equals("USER")) {
			return true;
		}
		return false;
